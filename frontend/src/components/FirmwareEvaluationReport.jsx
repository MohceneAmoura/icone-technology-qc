...
export default function FirmwareEvaluationReport({ firmwareId, firmwareName, deviceType, evaluationId, token, onClose }) {
  ...
  useEffect(() => {
    const fetchReport = async () => {
      try {
        let url;
        if (evaluationId) {
          url = `/api/firmware/evaluations/${evaluationId}`;
        } else {
          url = `/api/firmware/${firmwareId}/evaluation`;
        }
        console.log('Fetching firmware report from:', url);
        ...
        console.log('Fetched firmware report data:', data);
        ...
        console.error('Error fetching firmware report:', err);
  ...
  const handleExportPDF = () => {
    ...
    doc.setFontSize(18);
    doc.text('Icone Technology - Software (Firmware) Quality Control Report', 15, 16);
    
    doc.setTextColor(100, 116, 139);
    doc.setFontSize(8);
    doc.setFont('Helvetica', 'normal');
    doc.text('FIRMWARE QC REPORT', 240, 16);
    ...
    doc.setFont('Helvetica', 'bold');
    doc.text('Firmware Name:', 15, 45);
    doc.setFont('Helvetica', 'normal');
    doc.text(firmwareName, 42, 45);
    ...
    doc.save(`firmware_qc_report_${firmwareName.replace(/\s+/g, '_')}_${formattedDate}.pdf`);
  };
    ...
    <div className="modal-overlay">
      <div className="modal-content">
        <div className="modal-header">
          <h3>🔍 Software (Firmware) Quality Control Evaluation Report</h3>
        ...
        <div className="modal-body">
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '3rem', color: 'var(--text-secondary)' }}>
              <p>No evaluation report data available for this firmware.</p>
            </div>
          ) : (
            <div>
              {/* Info grid */}
              <div className="report-info-card">
                <div className="report-info-item">
                  <span className="label">Firmware Name</span>
                  <span className="val">{firmwareName}</span>
                </div>
                <div className="report-info-item">
